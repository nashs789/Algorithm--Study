...
package BaekJoon.Greedy.Q10162;
...
public class Q10162_Microwave {
    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        int foodSec = Integer.parseInt(br.readLine());
        int[] seconds = {300, 60, 10};
        int[] count = new int[3];

        for(int idx = 0; idx < seconds.length; idx++){
            if(foodSec / seconds[idx] > 0){
                count[idx] += foodSec / seconds[idx];
                foodSec %= seconds[idx];
            }
        }
